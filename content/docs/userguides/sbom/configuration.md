...
For each configurable value, Syft checks the following places in order, taking the first value that is found:

1. Arguments passed on the command line
2. Environment variables
3. The configuration file (see below for which file is chosen)
4. The default value

In other words, if `SYFT_LOG_LEVEL` is set in the environment, and `log.level` is set in the config file, and `-vvv` is passed, the log level will be `trace` because `-vvv` took precedence over the environment variable and the config file value.

If Syft is invoked with log level at debug or trace (`-vv` or `-vvv` or `SYFT_LOG_LEVEL=debug`), Syft prints its entire configuration at the beginning of its run.

Use `syft --help` for a list of command line arguments. Use `syft config` to print a sample config file to stdout. Use `syft config --load` to print the current config to stdout.
...
The path from the document root to the value is `format->json->pretty`, so the environment variable that sets this value `SYFT_FORMAT_JSON_PRETTY`. See below a complete sample config file with environment variables noted.

### Config File

The following paths are searched, in order, for a syft config file. The first file that exists is used entirely; no merging of config files is done at this time:
...
You can print a sample config file by running `syft config`. You can print the current config by running `syft config --load`.
